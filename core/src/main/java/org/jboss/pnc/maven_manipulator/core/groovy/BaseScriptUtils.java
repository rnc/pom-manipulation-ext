...
public abstract class BaseScriptUtils extends Script implements MavenBaseScript {
    ...
    /**
     * Allows the specified group:artifact property to be inlined. This is useful to split up properties that cover
     * multiple separate projects.
     *
     * @param currentProject The current project we are operating on.
     * @param groupArtifact A ProjectRef corresponding to the group and artifact of the dependency (or managed
     *        dependency) that we wish to inline.
    ...
    public void inlineProperty(Project currentProject, ProjectRef groupArtifact) throws ManipulationException {
        ...
        try {
            currentProject.getResolvedManagedDependencies(getSession())
                    ...
                    .stream()
                    .filter(
                            a -> (groupArtifact.getArtifactId().equals("*")
                                    && a.getKey().getGroupId().equals(groupArtifact.getGroupId())) ||
                                    (a.getKey().asProjectRef().equals(groupArtifact)
                                            && a.getValue().getVersion().contains("$")))
                    .forEach(a -> {
                        logger.debug("Found managed artifact {} (original dependency {})", a.getKey(), a.getValue());
                        a.getValue()
                                .setVersion(
                                        PropertyResolver.resolvePropertiesUnchecked(
                                                getSession(),
    ...
    /**
     * Allows the specified property to be inlined. This is useful to split up properties that cover multiple separate
     * projects.
     *
     * @param currentProject The current project we are operating on.
     * @param propertyKey The property which is within the dependencies (or managed dependencies) that we wish to
     *        inline.
     * @throws ManipulationException if an error occurs.
     */
    public void inlineProperty(Project currentProject, String propertyKey) throws ManipulationException {
        ...
        try {
            ...
            currentProject.getResolvedManagedPlugins(getSession())
                    ...
                    .stream()
                    .filter(
                            a -> (a.getValue().getVersion() != null)
                                    && (a.getValue().getVersion().equals("${" + propertyKey + "}")))
                    .forEach(a -> {
                        logger.debug("Found managed plugin {} (original dependency {})", a.getKey(), a.getValue());
                        a.getValue()
                                .setVersion(
    ...
    public void overrideProjectVersion(ProjectVersionRef gav) throws ManipulationException {
        ...
        if (thisMapping != null) {
            logger.info(
                    "Comparing requested GAV {} with target GAV build {} to this GAV {} with known build {}",
                    gav,
                    targetBuild,
                    getGAV(),
                    thisMapping);
                ...
                if (!suffixes.isEmpty()) {
                    ...
                    if (thisMapping.contains(suffix) && targetBuild.contains(suffix)
                            && Version.getIntegerBuildNumber(thisMapping) >= Version.getIntegerBuildNumber(
                                    targetBuild)) {
                        logger.error(
                                "Alignment failure: Target is {} and this is {}",
                                Version.getIntegerBuildNumber(targetBuild),
