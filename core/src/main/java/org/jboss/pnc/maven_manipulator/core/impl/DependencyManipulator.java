...
import static org.jboss.pnc.maven_manipulator.core.util.IdUtils.ga;
...
import org.jboss.pnc.maven_manipulator.common.model.SimpleScopedArtifactRef;
import org.jboss.pnc.maven_manipulator.common.util.PropertyResolver;
import org.jboss.pnc.maven_manipulator.common.util.WildcardMap;
import org.jboss.pnc.maven_manipulator.core.ManipulationSession;
import org.jboss.pnc.maven_manipulator.core.state.CommonState;
import org.jboss.pnc.maven_manipulator.core.state.DependencyState;
import org.jboss.pnc.maven_manipulator.core.state.DependencyState.DependencyPrecedence;
import org.jboss.pnc.maven_manipulator.core.state.RESTState;
import org.jboss.pnc.maven_manipulator.core.util.DependencyPluginUtils;
import org.jboss.pnc.maven_manipulator.core.util.DependencyPluginWrapper;
import org.jboss.pnc.maven_manipulator.core.util.PluginReference;
import org.jboss.pnc.maven_manipulator.core.util.PropertiesUtils;
import org.jboss.pnc.maven_manipulator.core.util.PropertyMapper;
import org.jboss.pnc.maven_manipulator.io.ModelIO;
import org.jboss.pnc.maven_manipulator.io.resolver.GalleyAPIWrapper;

/**
 * {@link Manipulator} implementation that can alter dependency (and dependency management) sections in a project's pom
 * file.
 * Configuration is stored in a {@link DependencyState} instance, which is in turn stored in the
...
public class DependencyManipulator extends CommonManipulator implements Manipulator {
    ...
    /**
     * Used to store mappings of old property to new version - the new version is encapsulated within the
     * {@link PropertyMapper}
     * which also contains reference to the old version and the dependency that changed this. This allows complete
     * tracking of
     * dependencies that updated properties - and therefore, the inverse, dependencies that did NOT update the property.
     * This can
    ...
    private Map<ArtifactRef, String> loadRemoteOverrides() throws ManipulationException {
        ...
        if (depState.getPrecedence() == DependencyPrecedence.BOM) {
            ...
            if (mergedOverrides.isEmpty()) {
                ...
                logger.warn(
                        "No dependencies found for dependencySource {}. Has {} been configured? ",
            ...
            if (mergedOverrides.isEmpty()) {
                logger.warn(
                        "No dependencies found for dependencySource {}. Has restURL been configured? ",
                        depState.getPrecedence());
            }
    ...
    private void removeDuplicateArtifacts(
            Map<ArtifactRef, String> mergedOverrides,
            Map<ArtifactRef, String> targetOverrides) {
        ...
        while (it.hasNext()) {
            ...
            for (final Entry<ArtifactRef, String> tatgetOverridesEntry : targetOverrides.entrySet()) {
                ...
                if (pRef.equals(target.asProjectRef())) {
                    logger.debug(
                            "Merging sources ; entry {}={} clashes (and will be removed) with precedence given to {}={}",
                            key,
                            mergedOverridesEntry.getValue(),
                            target,
                            tatgetOverridesEntry.getValue());
                    it.remove();
                    break;
                }
            }
        ...
        if (!result.isEmpty()) {
            ...
            for (final Entry<Project, Map<String, PropertyMapper>> e : versionPropertyUpdateMap.entrySet()) {
                ...
                for (final Entry<String, PropertyMapper> entry : map.entrySet()) {
                    ...
                    final PropertiesUtils.PropertyUpdate found = PropertiesUtils.updateProperties(
                            session,
                            project,
                            false,
                            key,
    ...
    private void apply(final Project project, final Model model, final Map<ArtifactRef, String> overrides)
        ...
        try {
            originalOverrides = applyModuleVersionOverrides(
                    projectGA,
                    dependencyState.getDependencyOverrides(),
                    originalOverrides,
                    explicitOverrides,
        ...
        if (project.isInheritanceRoot()) {
            ...
            if (project.getModelParent() != null) {
                for (Entry<ArtifactRef, String> entry : originalOverrides.entrySet()) {
                    ...
                    if (pvr.asProjectRef().equals(SimpleProjectRef.parse(ga(project.getModelParent())))) {
                        ...
                        logger.debug(
                                "Modifying parent reference from {} to {} for {}:{}",
                ...
                final Map<ArtifactRef, Dependency> pDepMap = Collections
                        .singletonMap(new SimpleScopedArtifactRef(d), d);
            ...
            if (commonState.isOverrideTransitive() && dependencyState.getRemoteBOMDepMgmt() != null) {
                ...
                logger.warn(
                        "Ignoring {} flag since it was used without the {} option",
    ...
    private void applyPlugins(
            Project project,
            Map<ProjectVersionRef, Plugin> plugins,
        ...
        for (PluginReference reference : refs) {
            if (reference.getVersion() != null) {
                String resolvedVersion = PropertyResolver
                        .resolveInheritedProperties(session, project, reference.getVersion());
                ProjectVersionRef resolvedRef = new SimpleProjectVersionRef(
     ...
     * @throws ManipulationException if an error occurs
     */
    private Map<ArtifactRef, String> applyOverrides(
            ...
            for (final Entry<ArtifactRef, String> entry : overrides.entrySet()) {
                ...
                if (depPr.equals(groupIdArtifactId)) {
                    ...
                    if (isEmpty(overrideVersion)) {
                        logger.warn(
                                "Unable to align with an empty override version for {}; ignoring",
                                groupIdArtifactId);
                    } else if (isEmpty(oldVersion)) {
                        logger.debug("Dependency is a managed version for {}; ignoring", groupIdArtifactId);
                    } else if (oldVersion.equals(Version.PROJECT_VERSION)
                    ...
                    // have immediate access to the original property so the closest that is feasible is verify strict matching.
                        ...
                        logger.debug(
                                "Original fully resolved version {} for {} does not match override version {} -> {} so ignoring",
                                resolvedValue,
                                dependency,
                                entry.getKey(),
                                overrideVersion);
                        if (commonState.isFailOnStrictViolation()) {
                            throw new ManipulationException(
                                    ...
                                    wrapper.getVersion(),
                                    resolvedValue,
                                    entry.getKey().getVersionString(),
                                    entry.getKey().asProjectRef().toString());
                        } else {
                            logger.warn(
                        ...
                        if (!PropertiesUtils.cacheProperty(
                                session,
                                project,
                                versionPropertyUpdateMap,
                                oldVersion,
                                overrideVersion,
                                entry.getKey(),
                                false)) {
                            if (strict && !PropertiesUtils.checkStrictValue(session, resolvedValue, overrideVersion)) {
                                ...
                                if (oldVersion.contains("${")) {
                                    ...
                                    logger.debug(
                                            "Resolved value is {} and replacement version is {}",
                                            resolvedValue,
                                            replaceVersion);
    ...
    private Map<ArtifactRef, String> removeReactorGAs(final Map<ArtifactRef, String> versionOverrides) {
        ...
        while (it.hasNext()) {
            ...
            if (session.getProjects()
                    .stream()
                    .anyMatch(
                            (p -> e.getKey().getGroupId().equals(p.getGroupId())
    ...
    private void validateDependenciesUpdatedProperty(
            CommonState cState,
            Project p,
        ...
        for (final Entry<ArtifactRef, Dependency> entry : dependencies.entrySet()) {
            ...
            if (startsWith(versionProperty, "${")) {
                PropertiesUtils.verifyPropertyMapping(
                        cState,
                        p,
                        versionPropertyUpdateMap,
                        pvr,
