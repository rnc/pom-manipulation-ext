...
import org.jboss.pnc.maven_manipulator.core.state.ProfileInjectionState;
...
/**
 * This Manipulator runs very early. It makes a REST call to an external service to increment the GAVs to align the
 * project version
 * and dependencies to. It will prepopulate Project GA versions into the VersioningState in case the
 * VersioningManipulator has been
...
public class RESTBOMCollector
    ...
    private void populateBOMVersions() throws RestException {
        ...
        if (restParam.size() > 0) {
            ...
            final ListIterator<ProjectVersionRef> emptyIterator = Collections.<ProjectVersionRef> emptyList()
                    .listIterator();

            // Process rest result for boms
            updateBOM(
                    (ds.getRemoteBOMDepMgmt() == null ? emptyIterator : ds.getRemoteBOMDepMgmt().listIterator()),
                    restResult);
            updateBOM(
                    (ps.getRemotePluginMgmt() == null ? emptyIterator : ps.getRemotePluginMgmt().listIterator()),
                    restResult);
            updateBOM(
                    (pis.getRemoteProfileInjectionMgmt() == null ? emptyIterator
                            : pis.getRemoteProfileInjectionMgmt().listIterator()),
                    restResult);
        } else {
    ...
    private void populateRestParam(
            final ArrayList<ProjectVersionRef> restParam,
            final String log,
            final List<ProjectVersionRef> bomMgmt) {
        asStream(bomMgmt).filter(
                ...
                .forEach(bom -> {
                    ...
                    ProjectVersionRef newBom = new SimpleProjectVersionRef(
                            bom.asProjectRef(),
    ...
    private void updateBOM(
            final ListIterator<ProjectVersionRef> iterator,
            final Map<ProjectVersionRef, String> restResult) {
        while (iterator.hasNext()) {
            ...
            if (!Version.hasBuildNumber(pvr.getVersionString()) && pvr.getVersionString()
                ...
                if (restResult.containsKey(newBom)) {
                    ProjectVersionRef replacementBOM = new SimpleProjectVersionRef(
                            pvr.asProjectRef(),
