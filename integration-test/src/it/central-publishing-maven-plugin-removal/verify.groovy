...
def plugin = pom.build.plugins.plugin.find { it.artifactId.text() == "central-publishing-maven-plugin" }
assert (plugin == null || plugin == "")

def profile = pom.profiles.children().find { it.id.text() == 'extra-plugins' }
def profileplugin = profile.build.plugins.plugin.find { it.artifactId.text() == "central-publishing-maven-plugin" }
