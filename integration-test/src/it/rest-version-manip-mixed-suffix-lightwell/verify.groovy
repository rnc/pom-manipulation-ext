...
assert pom.version.text().equals( '1.0.0.rhlw-00003-temporary-n-00002' )
...
assert dependency.version.text() == "1.0-temporary-n-1"

dependency = pom.dependencies.dependency.find { it.artifactId.text() == "errai-common" }
assert dependency != null
assert dependency.version.text() == "1.1-Final-temporary-n-1"

def passed = false
pom.properties.each {
    if ( it.text().contains ("3.1-temporary-n-1") )
    {
        passed = true
    }
}
