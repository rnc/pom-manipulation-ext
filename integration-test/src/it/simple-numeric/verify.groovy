...
// Verify metadata/src plugins are disabled by default
def plugin = pom.build.plugins.plugin.find { it.artifactId.text() == "project-sources-maven-plugin" }
assert (plugin == null || plugin == "")
plugin = pom.build.plugins.plugin.find { it.artifactId.text() == "buildmetadata-maven-plugin" }
assert (plugin == null || plugin == "")
